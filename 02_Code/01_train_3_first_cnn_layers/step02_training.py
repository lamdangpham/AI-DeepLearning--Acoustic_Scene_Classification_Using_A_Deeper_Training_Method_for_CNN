...
# 1.1/ Directory 
tf.flags.DEFINE_string("TRAIN_DIR",  "./../input_data/data_train/", "Directory of feature")
tf.flags.DEFINE_string("VALID_DIR",  "./../input_data/data_test/", "Directory of feature")


tf.flags.DEFINE_string("OUT_DIR",    "./data/",     "Point to output directory")

# 1.2/ Training para 
tf.flags.DEFINE_integer("N_TRAIN_MUL_BATCH", 26,     "Multi Batch Number for Training")   #74
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        with tf.control_dependencies(update_ops):
            ...
            # All trainable variables are put in 'finetune_vars'
            finetune_vars = list()
            finetune_vars.extend(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope="bn_01"))
            finetune_vars.extend(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope="conv01"))
            finetune_vars.extend(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope="conv02"))
            finetune_vars.extend(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope="conv03"))
            finetune_vars.extend(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope="conv04"))
            finetune_vars.extend(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope="dnn_01"))
            finetune_vars.extend(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope="dnn_02_01"))
            finetune_vars.extend(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope="dnn_02_02"))
            finetune_vars.extend(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope="dnn_02_03"))
            print('FINETUNED VARIABLES')
            print(finetune_vars)

            global_step    = tf.Variable(0, name="global_step", trainable=False)
            optimizer      = tf.train.AdamOptimizer(FLAGS.LEARNING_RATE)
            grads_and_vars = optimizer.compute_gradients(model.loss, var_list=finetune_vars)
        ...
        def test_process(x_test_batch):
            ...
            [step, end_output] = sess.run([global_step, model.output_layer], feed_dict)
